...
postCtrl.add = async (req, reply) =>{
    try{
       ...
       if (req.file) {newPost.imgUrl= `http://localhost:4000/public/${req.file.filename}` ;
       newPost.nameImage=req.file.filename
    }
...
postCtrl.delete = async (req,reply) => {
    try {
        ...
        if(!post){
            return response(reply,404,false,"","Post no encontrado para eliminar")}
